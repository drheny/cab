...
if __name__ == "__main__":
    ...
    def test_consultation_data_creation_verification(self):
        """Test 1: Verify Consultation Data Creation - All demo patients should have consultation histories"""
        print("
=== TESTING CONSULTATION DATA CREATION ===")
        
        # Test GET /api/consultations/patient/patient1 - should return 2+ consultations
        response = requests.get(f"{self.base_url}/api/consultations/patient/patient1")
        self.assertEqual(response.status_code, 200)
        patient1_consultations = response.json()
        self.assertIsInstance(patient1_consultations, list)
        self.assertGreaterEqual(len(patient1_consultations), 2, 
                               f"Patient1 (Yassine Ben Ahmed) should have 2+ consultations, found {len(patient1_consultations)}")
        print(f"✅ Patient1 (Yassine Ben Ahmed): {len(patient1_consultations)} consultations found")
        
        # Test GET /api/consultations/patient/patient2 - should return 2+ consultations  
        response = requests.get(f"{self.base_url}/api/consultations/patient/patient2")
        self.assertEqual(response.status_code, 200)
        patient2_consultations = response.json()
        self.assertIsInstance(patient2_consultations, list)
        self.assertGreaterEqual(len(patient2_consultations), 2, 
                               f"Patient2 (Lina Alami) should have 2+ consultations, found {len(patient2_consultations)}")
        print(f"✅ Patient2 (Lina Alami): {len(patient2_consultations)} consultations found")
        
        # Test GET /api/consultations/patient/patient3 - should return 2+ consultations
        response = requests.get(f"{self.base_url}/api/consultations/patient/patient3")
        self.assertEqual(response.status_code, 200)
        patient3_consultations = response.json()
        self.assertIsInstance(patient3_consultations, list)
        self.assertGreaterEqual(len(patient3_consultations), 2, 
                               f"Patient3 (Omar Tazi) should have 2+ consultations, found {len(patient3_consultations)}")
        print(f"✅ Patient3 (Omar Tazi): {len(patient3_consultations)} consultations found")
        
        return {
            "patient1": patient1_consultations,
            "patient2": patient2_consultations, 
            "patient3": patient3_consultations
        }
    
    def test_consultation_data_structure_validation(self):
        """Test 2: Test Consultation Data Structure - Verify each consultation has all required fields"""
        print("
=== TESTING CONSULTATION DATA STRUCTURE ===")
        
        # Get consultation data for all patients
        consultations_data = self.test_consultation_data_creation_verification()
        
        required_fields = ["id", "patient_id", "appointment_id", "date", "type_rdv", 
                          "observations", "traitement", "bilan", "duree", "poids", "taille", "pc"]
        
        for patient_id, consultations in consultations_data.items():
            print(f"
--- Validating {patient_id} consultations ---")
            
            for i, consultation in enumerate(consultations):
                # Verify all required fields are present
                for field in required_fields:
                    self.assertIn(field, consultation, 
                                f"{patient_id} consultation {i+1} missing field: {field}")
                ...
                self.assertIsInstance(consultation["date"], str)
                self.assertIsInstance(consultation["type_rdv"], str)
                self.assertIsInstance(consultation["observations"], str)
                self.assertIsInstance(consultation["traitement"], str)
                self.assertIsInstance(consultation["bilan"], str)
                ...
                try:
                    ...
                    self.fail(f"{patient_id} consultation {i+1} has invalid date format: {consultation[\"date\"]}")
                
                # Verify consultation type is valid
                self.assertIn(consultation["type_rdv"], ["visite", "controle"], 
                            f"{patient_id} consultation {i+1} has invalid type: {consultation[\"type_rdv\"]}")
                
                # Verify consultation content is not empty
                self.assertNotEqual(consultation["observations"].strip(), "", 
                                  f"{patient_id} consultation {i+1} has empty observations")
                self.assertNotEqual(consultation["traitement"].strip(), "", 
                                  f"{patient_id} consultation {i+1} has empty traitement")
                self.assertNotEqual(consultation["bilan"].strip(), "", 
                                  f"{patient_id} consultation {i+1} has empty bilan")
                
                print(f"✅ {patient_id} consultation {i+1}: {consultation[\"type_rdv\"]} on {consultation[\"date\"]}")
        
        print("✅ All consultation data structures validated successfully")
    
    def test_consultation_date_ranges_validation(self):
        """Test 3: Validate date ranges span different time periods"""
        print("
=== TESTING CONSULTATION DATE RANGES ===")
        ...
        for patient_id, consultations in consultations_data.items():
            print(f"
--- Validating {patient_id} date ranges ---")
            
            dates = [datetime.strptime(c["date"], "%Y-%m-%d") for c in consultations]
            dates.sort()
            
            # Verify we have consultations spanning different time periods
            self.assertGreater(len(dates), 1, f"{patient_id} should have multiple consultations")
            ...
            # Verify dates span reasonable time periods (at least 10 days apart)
            min_diff = min(date_diffs) if date_diffs else 0
            self.assertGreaterEqual(min_diff, 10, 
                                  f"{patient_id} consultations should be at least 10 days apart")
            
            # Verify we have historical data (at least one consultation older than 7 days)
            oldest_date = min(dates)
            days_ago = (today - oldest_date).days
            self.assertGreaterEqual(days_ago, 7, 
                                  f"{patient_id} should have consultations older than 7 days")
            
            print(f"✅ {patient_id}: Date range spans {days_ago} days, consultations {min_diff}+ days apart")
        
        print("✅ All consultation date ranges validated successfully")
    
    def test_consultation_types_validation(self):
        """Test 4: Confirm consultation types include both \"visite\" and \"controle\""""
        print("
=== TESTING CONSULTATION TYPES ===")
        ...
        for patient_id, consultations in consultations_data.items():
            ...
            for consultation in consultations:
                consultation_type = consultation["type_rdv"]
                patient_types.add(consultation_type)
                all_types.add(consultation_type)
            
            print(f"✅ {patient_id}: Has consultation types: {\", \".join(sorted(patient_types))}")
        
        # Verify we have both visite and controle types across all patients
        self.assertIn("visite", all_types, "Demo data should include \"visite\" consultations")
        self.assertIn("controle", all_types, "Demo data should include \"controle\" consultations")
        
        print(f"✅ All consultation types found: {\", \".join(sorted(all_types))}")
    
    def test_patient_consultation_navigation_workflow(self):
        """Test 5: Test Patient-Consultation Navigation Workflow"""
        print("
=== TESTING PATIENT-CONSULTATION NAVIGATION WORKFLOW ===")
        
        demo_patients = ["patient1", "patient2", "patient3"]
        patient_names = {
            "patient1": "Yassine Ben Ahmed",
            "patient2": "Lina Alami", 
            "patient3": "Omar Tazi"
        }
        ...
        for patient_id in demo_patients:
            print(f"
--- Testing workflow for {patient_id} ({patient_names[patient_id]}) ---")
            
            # Step 1: Verify patient data retrieval
            response = requests.get(f"{self.base_url}/api/patients/{patient_id}")
            self.assertEqual(response.status_code, 200)
            patient_data = response.json()
            ...
            self.assertIn("nom", patient_data)
            self.assertIn("prenom", patient_data)
            print(f"✅ Patient data retrieved: {patient_data[\"prenom\"]} {patient_data[\"nom\"]}")
            
            # Step 2: Test consultation history retrieval
            response = requests.get(f"{self.base_url}/api/consultations/patient/{patient_id}")
            self.assertEqual(response.status_code, 200)
            consultations = response.json()
            
            self.assertIsInstance(consultations, list)
            self.assertGreater(len(consultations), 0, f"No consultations found for {patient_id}")
            print(f"✅ Consultation history retrieved: {len(consultations)} consultations")
            
            # Step 3: Confirm data linkage between patients and consultations
            for consultation in consultations:
                self.assertEqual(consultation["patient_id"], patient_id, 
                               "Consultation patient_id should match requested patient")
                
                # Verify appointment_id linkage exists
                self.assertIn("appointment_id", consultation)
                self.assertNotEqual(consultation["appointment_id"], "", 
                                  "Consultation should have valid appointment_id")
            
            print(f"✅ Data linkage verified: All consultations properly linked to {patient_id}")
        
        print("✅ Patient-Consultation navigation workflow validated successfully")
    
    def test_phone_messages_consultation_navigation(self):
        """Test 6: Test Phone Messages → Consultation Navigation"""
        print("
=== TESTING PHONE MESSAGES → CONSULTATION NAVIGATION ===")
        
        # Simulate the workflow: phone message created for patient → navigate to consultation page
        test_cases = [
            {
                "patient_id": "patient1",
                "patient_name": "Yassine+Ben+Ahmed",
                "expected_consultations": 2
            },
            {
                "patient_id": "patient2", 
                "patient_name": "Lina+Alami",
                "expected_consultations": 2
            },
            {
                "patient_id": "patient3",
        ...
        for test_case in test_cases:
            ...
            expected_count = test_case["expected_consultations"]
            
            print(f"
--- Testing navigation for {patient_name.replace(\"+\", \" \")} ---")
            
            # Step 1: Simulate URL parameter passing from phone messages
            # Test URL parameter: ?patient=patient1&patientName=Yassine+Ben+Ahmed
            
            # Step 2: Verify consultation endpoint responds correctly
            response = requests.get(f"{self.base_url}/api/consultations/patient/{patient_id}")
            self.assertEqual(response.status_code, 200)
            consultations = response.json()
            
            # Step 3: Confirm consultation history loads
            self.assertIsInstance(consultations, list)
            self.assertGreaterEqual(len(consultations), expected_count, 
                                  f"Expected at least {expected_count} consultations for {patient_name}")
            
            # Step 4: Verify consultation endpoint responds with timestamp parameter
            timestamp = int(datetime.now().timestamp() * 1000)
            response = requests.get(f"{self.base_url}/api/consultations/patient/{patient_id}?_t={timestamp}")
            self.assertEqual(response.status_code, 200)
            consultations_with_timestamp = response.json()
            
            # Should return same data regardless of timestamp parameter
            self.assertEqual(len(consultations), len(consultations_with_timestamp))
            
            print(f"✅ Navigation successful: {len(consultations)} consultations loaded for {patient_name.replace(\"+\", \" \")}")
        
        print("✅ Phone Messages → Consultation navigation validated successfully")
    
    def test_consultation_history_expected_results(self):
        """Test 7: Verify Expected Results Match Specifications"""
        print("
=== TESTING EXPECTED RESULTS MATCH SPECIFICATIONS ===")
        
        expected_results = {
            "patient1": {
                "name": "Yassine Ben Ahmed",
                "expected_consultations": 2,
                "expected_dates": ["30 days ago", "15 days ago"]
            },
            "patient2": {
                "name": "Lina Alami", 
                "expected_consultations": 2,
                "expected_dates": ["45 days ago", "20 days ago"]
            },
            "patient3": {
                "name": "Omar Tazi",
                "expected_consultations": 2, 
                "expected_dates": ["today", "60 days ago"]
            }
        }
...
--- Verifying {expected[\"name\"]} ---")
            ...
            # Verify consultation count
            self.assertGreaterEqual(len(consultations), expected["expected_consultations"],
                                  f"{expected[\"name\"]} should have at least {expected[\"expected_consultations\"]} consultations")
            
            # Verify consultation dates are realistic
            consultation_dates = []
            for consultation in consultations:
                consultation_date = datetime.strptime(consultation["date"], "%Y-%m-%d")
                days_ago = (today - consultation_date).days
                consultation_dates.append(days_ago)
                
                # Verify consultation has realistic medical data
                self.assertGreater(len(consultation["observations"]), 20, 
                                 f"{expected[\"name\"]} consultation should have detailed observations")
                self.assertGreater(len(consultation["traitement"]), 10,
                                 f"{expected[\"name\"]} consultation should have treatment details")
                self.assertGreater(len(consultation["bilan"]), 10,
                                 f"{expected[\"name\"]} consultation should have bilan details")
            
            consultation_dates.sort()
            print(f"✅ {expected[\"name\"]}: {len(consultations)} consultations, dates: {consultation_dates} days ago")
            
            # Verify we have both recent and historical consultations
            self.assertGreater(max(consultation_dates), 10, 
                             f"{expected[\"name\"]} should have historical consultations (>10 days ago)")
        
        print("✅ All expected results verified successfully")
    
    def test_consultation_api_endpoints_comprehensive(self):
        """Test 8: Comprehensive API Endpoint Testing"""
        print("
=== COMPREHENSIVE API ENDPOINT TESTING ===")
        
        # Test 1: GET /api/consultations (all consultations)
        response = requests.get(f"{self.base_url}/api/consultations")
        self.assertEqual(response.status_code, 200)
        all_consultations = response.json()
        self.assertIsInstance(all_consultations, list)
        self.assertGreater(len(all_consultations), 0, "Should have demo consultations")
        print(f"✅ GET /api/consultations: {len(all_consultations)} total consultations")
        
        # Test 2: GET /api/consultations/patient/{patient_id} for each demo patient
        demo_patients = ["patient1", "patient2", "patient3"]
        total_patient_consultations = 0
        ...
        for patient_id in demo_patients:
            ...
            patient_consultations = response.json()
            self.assertIsInstance(patient_consultations, list)
            self.assertGreater(len(patient_consultations), 0, f"Patient {patient_id} should have consultations")
            total_patient_consultations += len(patient_consultations)
            print(f"✅ GET /api/consultations/patient/{patient_id}: {len(patient_consultations)} consultations")
        
        # Verify total counts match
        self.assertEqual(len(all_consultations), total_patient_consultations,
                        "Total consultations should match sum of patient consultations")
        
        # Test 3: Error handling - non-existent patient
        response = requests.get(f"{self.base_url}/api/consultations/patient/non_existent_patient")
        self.assertEqual(response.status_code, 404)
        print("✅ Error handling: 404 for non-existent patient")
        
        # Test 4: GET /api/consultations/{consultation_id} for individual consultations
        if len(all_consultations) > 0:
            test_consultation = all_consultations[0]
            ...
            # Verify detailed consultation includes patient and appointment info
            self.assertIn("patient", consultation_details)
            self.assertIn("appointment", consultation_details)
            print(f"✅ GET /api/consultations/{consultation_id}: Detailed consultation with patient/appointment info")
