    ...
    <div className="space-y-6">
      ...
      {activeTab === 'predictions' && (
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl shadow-sm border border-purple-200 p-6">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">Intelligence Artificielle Médicale</h2>
                  <p className="text-gray-600">Analyses ML avancées, prédictions saisonnières et revenus prévisionnels</p>
                </div>
              </div>
              <button
                onClick={() => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-white rounded-lg p-4 border border-blue-200 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-blue-600">Précision IA</p>
                    <p className="text-2xl font-bold text-blue-700">
                      {predictions?.next_month?.confiance || analysisData?.ai_confidence || 85}%
                    </p>
                  </div>
              ...
              <div className="bg-white rounded-lg p-4 border border-green-200 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-green-600">Modèle ML</p>
                    <p className="text-lg font-bold text-green-700">Gemini 2.0</p>
                    <p className="text-xs text-green-500">Flash Thinking</p>
                  </div>
              ...
              <div className="bg-white rounded-lg p-4 border border-orange-200 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-orange-600">Données analysées</p>
                    <p className="text-2xl font-bold text-orange-700">
                      {analysisData?.data_summary?.appointments_analyzed || 0}
                    </p>
                    <p className="text-xs text-orange-500">Consultations</p>
                  </div>
              ...
              <div className="bg-white rounded-lg p-4 border border-purple-200 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-purple-600">Score Global</p>
                    <p className="text-2xl font-bold text-purple-700">
                      {analysisData?.ai_analysis?.executive_summary?.overall_score || 0}/100
                    </p>
                  </div>
          ...
          {activityPeaks && activityPeaks.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-6 flex items-center">
                <DollarSign className="w-5 h-5 text-green-600 mr-2" />
                Prédictions de Revenus & Analyse Saisonnière
              </h3>
              
              {(() => {
                const peaksAnalysis = analyzeActivityPeaksAndLows(activityPeaks);
                const revenueForecast = generateRevenueForecast(peaksAnalysis?.analysis, revenuePredictions);
                ...
                return (
                  <div className="space-y-6">
                    ...
                    {revenueForecast && (
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {revenueForecast.slice(0, 3).map((forecast, index) => (
                          <div key={index} className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-lg p-4">
                            <div className="flex items-center justify-between mb-2">
                              <h4 className="text-sm font-semibold text-green-800">{forecast.month}</h4>
                              <div className={`px-2 py-1 rounded-full text-xs font-medium ${
                                forecast.trend === 'up' ? 'bg-green-200 text-green-800' :
                                forecast.trend === 'down' ? 'bg-red-200 text-red-800' : 'bg-gray-200 text-gray-800'
                              }`}>
                                {forecast.trend === 'up' ? '📈 Hausse' : forecast.trend === 'down' ? '📉 Baisse' : '➡️ Stable'}
                              </div>
                            </div>
                            <p className="text-2xl font-bold text-green-700">
                              {forecast.predictedRevenue.toFixed(0)} TND
                            </p>
                            <div className="flex items-center mt-2">
                              ...
                              <span className="text-xs text-green-600 ml-2">{forecast.confidence.toFixed(0)}% fiable</span>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    {/* Activity Peaks and Lows Analysis */}
                    {peaksAnalysis && (
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="bg-gradient-to-br from-orange-50 to-red-50 border border-orange-200 rounded-lg p-4">
                          <h4 className="text-md font-semibold text-orange-800 mb-3 flex items-center">
                            ...
                            Périodes de Pics d'Activité
                          </h4>
                          <div className="space-y-2">
                            {peaksAnalysis.peaks.slice(0, 3).map((peak, index) => (
                              <div key={index} className="flex items-center justify-between p-2 bg-white rounded border border-orange-100">
                                <div className="flex items-center space-x-2">
                                  ...
                                  <div>
                                    <p className="font-medium text-gray-800">{peak.month}</p>
                                    <p className="text-xs text-gray-600">{peak.consultations} consultations</p>
                                  </div>
                                </div>
                                <div className="text-right">
                                  <p className="font-bold text-orange-600">{peak.revenue.toFixed(0)} TND</p>
                                  <p className="text-xs text-orange-500">
                                    +{(((peak.consultations / peaksAnalysis.averages.consultations) - 1) * 100).toFixed(0)}%
                                  </p>
                                </div>
                              </div>
                            ))}
                        ...
                        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4">
                          ...
                          <div className="space-y-2">
                            {peaksAnalysis.lows.slice(0, 3).map((low, index) => (
                              <div key={index} className="flex items-center justify-between p-2 bg-white rounded border border-blue-100">
                                <div className="flex items-center space-x-2">
                                  ...
                                  <div>
                                    <p className="font-medium text-gray-800">{low.month}</p>
                                    <p className="text-xs text-gray-600">{low.consultations} consultations</p>
                                  </div>
                                </div>
                                <div className="text-right">
                                  <p className="font-bold text-blue-600">{low.revenue.toFixed(0)} TND</p>
                                  <p className="text-xs text-blue-500">
                                    {(((low.consultations / peaksAnalysis.averages.consultations) - 1) * 100).toFixed(0)}%
                                  </p>
                                </div>
                              </div>
                            ))}
          ...
          {/* Seasonal Predictions Chart */}
          {activityPeaks && activityPeaks.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                <BarChart3 className="w-5 h-5 text-purple-600 mr-2" />
                Analyse Saisonnière ML - Consultations & Revenus
              </h3>
              ...
              {(() => {
                ...
                return peaksAnalysis ? (
                  <div className="h-64">
                    <Line
                      data={{
                        labels: peaksAnalysis.analysis.map(item => item.month),
                        datasets: [
                          {
                            label: 'Consultations',
                            data: peaksAnalysis.analysis.map(item => item.consultations),
                            borderColor: 'rgb(147, 51, 234)',
                            backgroundColor: 'rgba(147, 51, 234, 0.1)',
                            tension: 0.4,
                            fill: false,
                          ...
                          {
                            label: 'Revenus (TND)',
                            data: peaksAnalysis.analysis.map(item => item.revenue),
                            borderColor: 'rgb(34, 197, 94)',
          ...
          {/* Top Patients Section - Enhanced */}
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
              <Award className="w-5 h-5 text-yellow-600 mr-2" />
              Top Patients Rentables (ML Analysis)
            </h3>
            <TopPatientsWidget />
          </div>
          ...
          {analysisData && (
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl shadow-sm border border-indigo-200 p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {analysisData.ai_analysis?.opportunities && (
                  <div className="space-y-4">
                    <h4 className="font-semibold text-indigo-800">Opportunités Stratégiques</h4>
                    {Object.entries(analysisData.ai_analysis.opportunities).map(([category, opportunities]) => (
                      <div key={category} className="bg-white rounded-lg p-4 border border-indigo-100">
                        <h5 className="font-medium text-indigo-700 mb-2 capitalize">
                          {category.replace('_', ' ')}
                        </h5>
                        <ul className="text-sm text-gray-700 space-y-1">
                          {Array.isArray(opportunities) ? opportunities.slice(0, 3).map((opportunity, index) => (
                ...
                {analysisData.ai_analysis?.risk_assessment && (
                  <div className="space-y-4">
                    <h4 className="font-semibold text-red-800">Évaluation des Risques</h4>
                    {Object.entries(analysisData.ai_analysis.risk_assessment).map(([category, risks]) => (
                      <div key={category} className="bg-white rounded-lg p-4 border border-red-100">
                        <h5 className="font-medium text-red-700 mb-2 capitalize">
                          {category.replace('_', ' ')}
                        </h5>
                        <ul className="text-sm text-gray-700 space-y-1">
                          {Array.isArray(risks) ? risks.slice(0, 3).map((risk, index) => (
                            <li key={index} className="flex items-start">
          ...
          {loading && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12">
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="mt-4 text-lg font-medium text-gray-700">Analyse ML en cours...</p>
                <p className="text-sm text-gray-500">Gemini 2.0 Flash traite vos données médicales</p>
              </div>
            </div>
          )}
          ...
          {/* Empty State */}
          {!loading && 
           (!predictions || predictions.status === 'no_data' || predictions.status === 'error') && 
           (!analysisData || analysisData.status === 'no_data' || analysisData.status === 'error') && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12">
              <div className="text-center">
                <Brain className="w-16 h-16 text-gray-400 mx-auto mb-4" />
                <h3 className="text-lg font-medium text-gray-900 mb-2">
                  {predictions?.status === 'error' || analysisData?.status === 'error' 
                    ? 'Erreur lors du chargement des données ML' 
                    : 'Aucune analyse ML disponible'}
                </h3>
                <p className="text-gray-500 mb-6">
                  {predictions?.status === 'error' || analysisData?.status === 'error'
                    ? 'Vérifiez la connexion et réessayez'
                    : 'Lancez une analyse pour obtenir des prédictions avancées'}
                </p>
                <button
                  onClick={() => {
                    setLoading(true);
                  ...
                  className="bg-gradient-to-r from-purple-500 to-indigo-600 text-white px-6 py-3 rounded-xl hover:from-purple-600 hover:to-indigo-700 transition-all duration-300 flex items-center space-x-2 mx-auto"
                >
                  <Brain className="w-5 h-5" />
                  <span>
                    {predictions?.status === 'error' || analysisData?.status === 'error' 
                      ? 'Réessayer l\'analyse ML' 
                      : 'Lancer l\'analyse ML'}
                  </span>
                </button>
              </div>
